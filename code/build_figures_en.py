#!/usr/bin/env python3
"""Английские версии шести иллюстраций рукописи — для препринта arXiv.

    python 09-tools/build_figures_en.py

Копия `build_figures.py` с переведёнными надписями. Геометрия, данные и
вычисления не менялись: скрипт только читает и рисует, ни одно число не
пересчитывается. Русские значения из CSV (имена процедур, единицы шкал)
переводятся картами PROC_EN / UNIT_EN; неизвестное значение останавливает
сборку, а не проходит молча.

Выход: `08-paper/figures-en/fig-NN-*.png` и `.pdf`, манифест
`08-paper/figures-en/figures-manifest.json`.
"""
...
PROC_EN = {
    "1 — индекс стиля score-v2": ("1", "style index score-v2"),
    "2 — классификатор P2b": ("2", "classifier P2b"),
    "3 — zero-shot nll-v2": ("3", "zero-shot nll-v2"),
    "4 — модель-судья judge-v2": ("4", "judge model judge-v2"),
}
UNIT_EN = {
    "пункты индекса 0–100": "index points, 0–100",
    "вероятность класса A, 0–1": "probability of class A, 0–1",
    "средний token NLL": "mean token NLL",
    "шкала рубрики 0–100": "rubric scale, 0–100",
}
...
def save(fig, number, slug):
    ...
    plt.close(fig)
    print(f"  figure {number}: fig-{number:02d}-{slug}")
...
def fig1():
    ...
    box(0.01, 0.60, 0.30, 0.30,
        f"Machine part\n45 tasks × 4 channels\n× 3 modes × 2 replicates\n{machine} documents",
        fc="#f4f1ee", fs=7.8)
    box(0.01, 0.12, 0.30, 0.30,
        "Human part\narchive before 2022-01-01\nregimentation level\nassigned by source",
        fc="#f4f1ee", fs=7.8)

    box(0.375, 0.36, 0.235, 0.30,
        "Preprocessing\nprep-v5\ntwo profiles:\nprose and full", fc="white", fs=7.8)

    box(0.68, 0.60, 0.30, 0.30,
        f"Analytical corpus\n{machine + human} documents\nA = {machine}, H = {human}",
        fc="#f0efe8", weight="bold")
    box(0.68, 0.12, 0.30, 0.30,
        "34 excluded\nall human\nshorter than 700 words\nafter extraction correction",
        fc="white", ec=SOFT)
    ...
    ax.text(0.5, 0.02, f"Human strata: {genres}", ha="center",
...
def fig2():
    ...
    for ax, r in zip(axes, rows):
        ...
        num, name = PROC_EN[r["procedure"]]
        ax.set_title(f"{num}. {name}", fontsize=8)
        ax.set_xticks([])
        ax.set_ylabel(UNIT_EN[r["unit"]], fontsize=7.5)
        ax.tick_params(labelsize=7.5)
        ax.text(0.5, -0.06, f"{est:+.4g}   p = {float(r['p_value']):.4f}\n"
                f"sign after convention: {sign.strip()}",
                transform=ax.transAxes, ha="center", va="top",
                fontsize=7.5, color=colour)
    fig.suptitle("Contrast P3 − P1, estimand full: four operationalisations, each on its own scale",
                 fontsize=9.5, y=1.30)
    fig.text(0.5, 1.19,
             "Panels use different scales: each procedure has its own unit.\n"
             "Magnitudes are not compared across procedures — what is compared "
             "is the sign and whether the interval covers zero.\n"
             "The value shown is the raw metric; the line “sign after convention” "
             "refers to the general reading rule\n“higher means more AI-like” "
             "and for procedure 3 does not match the sign of the raw value.",
...
def fig3():
    ...
    ax.text(median(fpr), 1.01, f" median {median(fpr):.4f}", transform=tr,
            ha="left", va="bottom", fontsize=7.5, color=MUTED)
    ax.set_xlabel("False positive rate on human texts, 95% interval")
    ax.set_title("The cost of error is unevenly distributed: 18 group holdouts",
                 fontsize=9.5, pad=16)
...
def fig4():
    ...
    ax1.set_xlabel("ECE, 10 equal-frequency bins")
    ax1.set_title("Calibration is heterogeneous", fontsize=9.5)
    ...
    ax2.plot(cov, macro, "o-", color=INK, markersize=4, linewidth=1.2,
             label="macro over 18 holdouts")
    ax2.plot(cov, pooled, "s--", color=MUTED, markersize=3.5, linewidth=1.0,
             label="pooled, sensitivity")
    ax2.set_xlabel("Coverage"); ax2.set_ylabel("Share of erroneous decisions")
    ax2.set_title("Risk–coverage: no threshold selected", fontsize=9.5)
...
def fig5():
    ...
    names = {1: "t01 em-dash replacement", 2: "t02 markdown change", 3: "t03 heading removal",
             4: "t04 paragraph reordering", 5: "t05 punctuation", 10: "t10 shortening",
             11: "t11 expansion", 13: "t13 spelling errors", 15: "t15 homoglyphs",
             16: "t16 format transfer"}
    stats = []
    ...
    ax.barh([s[0] for s in stats], [s[1] for s in stats], height=0.55,
            color=[ACCENT if s[1] > 0.1 else SOFT for s in stats], label="decision flip")
    ax.plot([s[2] for s in stats], list(y), "o", color=INK, markersize=4,
            label="instability |Δp| > 0.05")
    ax.set_xlabel("Mean per-document share of eligible models (464 pairs in total)")
    ax.set_title("Robustness of the decision to ten transformations, procedure 2", fontsize=9.5)
...
def fig6():
    ...
    steps = ["Emphasis markers\nremoved",
             f"full profile\nunchanged\n{num[0]} = {num[1]} characters",
             f"heading/prose boundary\nshifted\nprose {num[2]} → {num[3]}",
             "Format counters:\nemphasis spans 76 → 0",
             "The decision of all\nsix models\nflipped"]
    ...
    ax1.set_title("The effect acts through the pipeline,\nnot through the words", fontsize=9)
    ...
    ax2.text(1.02, 0.5, " threshold", fontsize=7.5, color=MUTED, va="center")
    ax2.set_xticks([0, 1]); ax2.set_xticklabels(["before", "after"])
    ax2.set_ylabel("Probability of machine origin")
    ax2.set_ylim(-0.05, 1.05)
    ax2.set_title(f"All {len(scores)} applicable models\nflipped their decision", fontsize=9)
...
def main():
    ...
    print(f"building English figures, {stamp}")
    ...
    manifest = {
        "series": "figures-v1-en",
        "derived_from": "09-tools/build_figures.py — та же геометрия и те же данные, надписи переведены",
        "data_series": "v2",
    ...
    print(f"  inputs pinned: {len(INPUTS)}, output files: {len(outputs)}")
    print(f"  manifest: {(OUT / 'figures-manifest.json').relative_to(ROOT)}")
